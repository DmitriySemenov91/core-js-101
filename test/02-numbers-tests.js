...
describe('02-numbers-tasks', () => {
  ...
  it.optional('getParallelipidedDiagonal should return a diagonal length of the rectagular parallepiped', () => {
    assert.equal(Math.sqrt(3), tasks.getParallelepipedDiagonal(1, 1, 1));
    assert.equal(Math.sqrt(27), tasks.getParallelepipedDiagonal(3, 3, 3));
    assert.equal(Math.sqrt(14), tasks.getParallelepipedDiagonal(1, 2, 3));
  });
